...
<section id="events">
    <div class="heading">
        <h1>
            <?php
            $categoryy_name = $_GET["category_name"];
            $label = "SELECT category_name FROM category WHERE category_name='$categoryy_name'";
            $querylabel = mysqli_query($connection,$label);
            $labelresult = mysqli_fetch_array($querylabel);
            echo $labelresult['category_name'];
            ?>
        </h1>
    </div>
...
<?php
$categoryy_name = $_GET["category_name"];
$displayqueryy = "SELECT event_name,event_description,event_photo,event_id,event_cost,event_name,event_date FROM eventss WHERE category_name='$categoryy_name'";
$querydisplayy = mysqli_query($connection,$displayqueryy);
while($result = mysqli_fetch_array($querydisplayy))
{
?>
    <div class="item">
        <div class="image">
            <a href="./eventDetails.php?event_id=<?php echo $result['event_id'];?>"><img src="<?php echo $result['event_photo'];?>" alt=""></a>
        </div>
